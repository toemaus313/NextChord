...
→ You might need to enable entitlements in Xcode (macOS) or manifest in Android Studio. Windsurf can help with platform-specific setup.

**"Build runner not generating code"**
→ Run `flutter pub run build_runner clean` then `build_runner build` again.

**"Hot reload not reflecting changes"**
→ Sometimes you need a full rebuild: `flutter run --no-fast-start`
...
4. **Read generated code**. Drift auto-generates code—understanding it helps.

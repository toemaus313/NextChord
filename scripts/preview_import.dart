...
String convertToChordPro(String rawData, String title, String artist, String key, String timeSignature, String? tempo) {
...
void main() async {
  ...
  try {
    ...
    print('✅ Selected ${selectedSongs.length} random songs for import preview\n');
    print('=' * 80);
    ...
    output.writeln('=' * 80);
    output.writeln();
    output.writeln('This file shows 5 randomly selected songs from the Justchords library.json');
    output.writeln('and how they would be converted to NextChord\'s ChordPro format.');
    output.writeln();
    ...
    for (var i = 0; i < selectedSongs.length; i++) {
      ...
      final rawData = song['rawData'] as String? ?? '';
      final timeSignature = (song['timeSignature'] as String? ?? '4/4').replaceAll(r'\/', '/');
      ...
      // Convert to ChordPro
      final chordPro = convertToChordPro(rawData, title, artist, key, timeSignature, tempo);

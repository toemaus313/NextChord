...
class _MidiProfilesModalState extends State<MidiProfilesModal> {
  ...
  void _selectProfile(MidiProfile? profile) {
    setState(() {
      ...
      if (profile != null) {
        ...
        // Convert stored profile back to display format
        _controlChanges = <MidiCC>[];
        ...
        _controlChanges.addAll(profile.controlChanges);
      } else {
        _clearForm();
      }
    });
  ...
  void _addControlChange() {
    ...
    try {
      // Handle timing keyword
      if (text.toUpperCase() == 'TIMING') {
        setState(() {
          _timing = true;
          _controlChangeController.clear();
          _notesController.clear();
      ...
      if (cc != null) {
        ...
        _showError(
            'Invalid format. Use "CC0:127" for Control Change, "PC5" for Program Change, or "timing" for MIDI clock');
      }
    } catch (e) {
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      // Extract program change from control changes (controller: -1 indicates PC)
      int? programChangeNumber;
      final controlChanges = <MidiCC>[];

      for (final cc in _controlChanges) {
        if (cc.controller == -1) {
          programChangeNumber = cc.value;
        } else {
          controlChanges.add(cc);
        }
      }
      ...
      final profile = MidiProfile(
        ...
        createdAt: _selectedProfile?.createdAt ?? DateTime.now(),
  ...
  Future<void> _testProfileMidiCommands() async {
    ...
    final hasCommands =
        programChange != null || controlChanges.isNotEmpty || _timing;
  ...
  Widget _buildHeader(BuildContext context) {
    return Row(
      children: [
        ...
        const Spacer(),
        // Test button (upper right)
        TextButton(
          onPressed: _testProfileMidiCommands,
          style: TextButton.styleFrom(
            foregroundColor: Colors.white,
            padding: const EdgeInsets.symmetric(horizontal: 21, vertical: 11),
            minimumSize: const Size(0, 0),
